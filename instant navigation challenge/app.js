...
function AuthorTab({ isActive, author, setSelectedAuthor }) {
  ...
  return (
    <button
      ...
      onMouseEnter={() => queryClient.prefetchQuery(booksByAuthorQuery(author))}
    >
      {author}
    </button>
  );
...
function BookDetailView({ bookId, author }) {
  const { data } = useBook(bookId, author);
...
export default function App() {
  ...
  return (
    <React.Fragment>
      ...
      {bookId ? (
        <BookDetailView bookId={bookId} author={selectedAuthor}/>
      ) : (
        <BookListView
          setBookId={setBookId}
          selectedAuthor={selectedAuthor}
